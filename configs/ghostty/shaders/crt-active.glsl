...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    float bloomStr     = BLOOM;                                 // 0→1 direct
    float bloomRad     = mix(4.0, 24.0, BLOOM_RADIUS);         // pixel spread: 4px tight → 24px wide
    ...
    // --- Bloom ---
    // Blurs the image with bloomRad spread, then adds only the excess where
    // neighbors are brighter than the current pixel — glow bleeds from bright
    // text into surrounding dark pixels without brightening the whole screen.
    if (bloomStr > 0.0) {
        vec3 bloom = vec3(0.0);
        float total = 0.0;
